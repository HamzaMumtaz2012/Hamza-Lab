...
function AddToCartButton() {
  return (
      <button className="add-to-cart mt-3 inline-flex h-10 w-10 items-center gap-2 overflow-visible rounded-full border-0 bg-[#ffc107] px-2.75 text-black transition-[width,background-color,color] duration-200 hover:w-35.25 hover:bg-[#ffb300] hover:text-black" type="button" aria-label="Add to cart">
     ...
     <iframe width="" height="315" src="https://www.youtube.com/embed/6S6pFHXROXA?si=3PCGaoBcbS3eBRyA" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" referrerpolicy="strict-origin-when-cross-origin" allowfullscreen></iframe>
      ...
      <section className="mx-auto mb-28 w-full max-w-[1263px] px-6 max-md:mb-[60px] max-md:px-3" id="services">
        <div className="grid grid-cols-4 gap-7 max-md:grid-cols-2 max-md:gap-3.5">
          {serviceCards.map(([title, image]) => (
            <a className="group relative aspect-[1.18] overflow-hidden rounded border border-red-500 bg-red-400 text-white" href="#lab-tests" key={title}>
      ...
      <section className="mx-auto mt-24 w-full max-w-[1263px] px-6 max-md:mt-16 max-md:px-3" id="lab-tests">
        <div className="mb-7 flex items-center justify-center gap-4 max-md:flex-col max-md:items-start max-md:gap-2.5">
          <h1 className="m-0 text-[clamp(24px,2.4vw,31px)] leading-tight text-red-600 max-md:text-[23px]">Lab Tests With Free Home Collection</h1>
          <a className="rounded-[5px] bg-yellow-400 px-3 py-2 text-[13px] text-white no-underline" href="#lab-tests">All Lab Tests</a>
        </div>
        <div className="grid grid-cols-4 gap-7 max-md:grid-cols-2 max-md:gap-3.5">
      ...
      <section className="mt-28 bg-[#f5faff] px-6 py-20 max-md:mt-20 max-md:px-3 max-md:py-14">
        <div className="mx-auto w-full max-w-[1263px]">
          <h2 className="mb-10 text-center text-[clamp(25px,3vw,36px)] font-bold text-red-600">Trusted by Over 10 Million Customers Across Pakistan</h2>
          <div className="grid grid-cols-3 gap-6 max-md:grid-cols-1">
            {testimonials.map(([name, quote]) => (
              <article className="border border-[#e0eaf2] bg-white p-7 shadow-[0_8px_25px_rgba(26,92,135,0.06)]" key={name}>
                <div className="mb-4 text-lg tracking-[3px] text-yellow-400">★★★★★</div>
      ...
      <section className="mx-auto grid w-full max-w-[1263px] grid-cols-4 gap-5 px-6 py-20 max-md:grid-cols-2 max-md:gap-3 max-md:px-3 max-md:py-14">
        {["Free Home Collection", "24/7 Support Online", "Fastest Turnaround Time", "Reliable & Trusted"].map((feature, index) => (
          <div className="border-r border-[#dce6ed] px-4 text-center last:border-0 max-md:border-b max-md:border-r-0 max-md:pb-5" key={feature}>
            <span className="mb-4 inline-grid h-12 w-12 place-items-center rounded-full bg-yellow-100 text-xl font-bold text-red-400">{index + 1}</span>
      ...
      <section className="mx-auto grid w-full max-w-[1263px] grid-cols-2 gap-16 px-6 pb-24 max-md:grid-cols-1 max-md:gap-10 max-md:px-3 max-md:pb-16">
        <div>
          <h2 className="mb-7 text-3xl font-bold text-red-600">Frequently Asked Questions</h2>
            ...
            {faqs.map(([question, answer]) => (
              <details className="group py-5" key={question}>
                <summary className="flex cursor-pointer list-none items-center justify-between text-sm font-bold text-[#273449] marker:hidden">
                  {question}<span className="text-xl font-normal text-yellow-400 group-open:rotate-45">+</span>
                </summary>
        ...
        <div className="flex flex-col justify-center  bg-red-500 px-10 py-12 text-white max-md:px-6 rounded-xl">
          ...
          <a className="mt-5 w-fit rounded bg-yellow-400 px-5 py-3 text-sm font-bold text-white no-underline hover:bg-yellow-500" href="tel:+9221111786986">Call +92 (021) 111786986</a>
        </div>
      </section>
      ...
      <section className="bg-[#f6f7f7] px-6 py-14 text-black max-md:px-3">
        <div className="mx-auto flex w-full max-w-[800px] flex-col items-center text-center">
          ...
          <form className="mt-7 flex w-full max-w-[560px] gap-2 max-md:flex-col" onSubmit={(event) => event.preventDefault()}>
            ...
            <button className=" bg-yellow-400 px-6 py-3 text-sm font-bold text-red-600 hover:bg-yellow-500 rounded-full cursor-pointer " type="submit">SUBSCRIBE</button>
          </form>
        </div>
      </section>
